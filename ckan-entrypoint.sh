...
if [ -z "$CKAN_SQLALCHEMY_URL" ]; then
  if ! CKAN_SQLALCHEMY_URL=$(link_postgres_url); then
    ...
    for tries in $(seq 30); do
      echo "Trying to connect to Postgres for the $tries time ..."
...
echo "Installing the custom plugins ..."
for REPO in $(ls $CKAN_HOME/src/plugins)
do
  cd $CKAN_HOME/src/plugins/$REPO && ckan-pip install -e .
done
